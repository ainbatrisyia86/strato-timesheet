<section>
    ...
    <form method="post" action="{{ route('password.update') }}" 
       x-data="{ 
            ...
            // Function to verify current password against backend
            async checkCurrentPassword() {
                if (this.current_password.length === 0) return;
                try {
                    const response = await fetch('{{ route('password.check') }}', {
                        ...
                        body: JSON.stringify({ current_password: this.current_password })
                    });
                    const data = await response.json();
                    this.currentPasswordCorrect = data.match;
                } catch (e) {
            ...
            get canSave() { return this.current_password.length > 0 && this.currentPasswordCorrect && this.isPasswordValid && this.isMatch }
        }"
        ...
        <div x-data="{ show: false }">
            ...
            <div class="mt-1 flex items-center w-full h-10 border rounded-md shadow-sm overflow-hidden transition-all duration-200 bg-white"
                :style="current_password.length > 0 ? 
                        (currentPasswordCorrect ? 'border-color: #D1D5DB' : 'border-color: #ef4444; border-width: 2px') 
                        : 'border-color: #D1D5DB'">
                ...
                <input 
                    ...
                    @input="currentPasswordCorrect = true"
            ...
            <p x-show="!currentPasswordCorrect && current_password.length > 0" 
